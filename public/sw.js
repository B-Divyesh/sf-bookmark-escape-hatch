...
self.addEventListener('fetch', (event) => {
  ...
  if (event.request.mode === 'navigate') {
    event.respondWith(fetch(event.request)
      ...
      .catch(async () => (await caches.match(event.request, { ignoreSearch: true, ignoreVary: true })) || (await caches.match('/index.html', { ignoreVary: true })) || (await caches.match('/offline.html', { ignoreVary: true }))));
    return;
  }
